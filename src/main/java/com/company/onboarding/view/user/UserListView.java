...
public class UserListView extends StandardListView<User> {
    ...
    @ViewComponent("usersDataGrid.edit")
    private EditAction<User> usersDataGridEdit;
    @Autowired
    private EditedEntityTransformer editedEntityTransformer;
    ...
    public void onUsersDataGridItemDoubleClick(final ItemDoubleClickEvent<User> event) {
        ListDataComponent<User> item = usersDataGridEdit.getTarget();
        assert item != null;
        if (item.getSingleSelectedItem() != null) usersDataGridEdit.execute();
    }
}